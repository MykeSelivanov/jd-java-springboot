...
import com.restwebflux.entity.Ticket;
...
public interface TicketRepository extends JpaRepository<Ticket,Long> {
    ...
    List<Ticket> fetchAllTicketByUserJPQL(long userId);
    ...
    @Query(value = "SELECT * FROM ticket JOIN user_account ua ON ticket.user_account_id=ua.id " +
            ...
            " WHERE ua.username ILIKE concat('%',?1,'%') OR ad.name ILIKE concat('%',?1,'%') or m.name ILIKE concat('%',?1,'%')",
    nativeQuery = true)
